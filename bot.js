...
bot.on('message', async (msg) => {
  ...
  if (s.state === 'awaiting_admin_password') {
    if (text === ADMIN_PASSWORD) {
      s.state = 'admin';
      bot.sendMessage(chatId, '✦ 𖤐 ADMIN PANEL 𖤐 ✦\n\n🟢 /pending — Pending orders\n🔵 /pendingdep — Pending deposits\n🟣 /users — Users list\n🔴 /exit — Exit admin');
    } else {
  ...
  if (s.state === 'awaiting_wp') {
    ...
    const order = {
      orderId: orderId, chatId: chatId, username: user.username,
  ...
  if (s.state === 'awaiting_utr') {
    ...
    const dep = {
      id: depId, chatId: chatId, username: user.username,
